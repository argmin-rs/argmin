...
pub fn rosenbrock_2d<T: Float + FromPrimitive>(param: &[T; 2], a: T, b: T) -> T {
    let [x, y] = *param;
    (a - x).powi(2) + b * (y - x.powi(2)).powi(2)
}

/// Derivative of 2D Rosenbrock function
pub fn rosenbrock_2d_derivative<T: Float + FromPrimitive>(param: &[T; 2], a: T, b: T) -> [T; 2] {
...
pub fn rosenbrock_2d_hessian<T: Float + FromPrimitive>(param: &[T; 2], _a: T, b: T) -> [[T; 2]; 2] {
    let num2 = T::from_f64(2.0).unwrap();
    let num4 = T::from_f64(4.0).unwrap();
    let num12 = T::from_f64(12.0).unwrap();
    let [x, y] = *param;
    [
        [num12 * b * x.powi(2) - num4 * b * y + num2, -num4 * b * x],
        [-num4 * b * x, num2 * b],
    ]
}
...
mod tests {
    ...
    fn test_rosenbrock_derivative() {
        let res = rosenbrock_2d_derivative(&[1.0, 1.0], 1.0, 100.0);
        for elem in &res {
            assert!((elem - 0.0).abs() < std::f64::EPSILON);
        }
        let res = rosenbrock_2d_derivative(&[1.0_f32, 1.0_f32], 1.0_f32, 100.0_f32);
        for elem in &res {
